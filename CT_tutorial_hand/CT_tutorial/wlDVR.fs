...
const float global_opacity = 200.0; 
...
void main()
{
	float level2 = level1+0.01;
	float start,final;
	float ddd = step_length*global_opacity;
	vec4 color = vec4(0.0);						
	...
	ps+=step;
			...
			for(float ii=0.0;ii<2000.0 && (ps.w<final);ii+=0.1)
			{
				...
				if(e>level1)
				{
					vec4 cl = vec4(clamp((e-level1)/(level2-level1),0.0,1.0));
										
					float d_alpha = (1.0-color.w) * cl.w * ddd;
